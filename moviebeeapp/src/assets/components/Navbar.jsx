    ...
    <header className=' sticky top-0 z-30 bg-black text-white'>
      <nav className=' flex items-center justify-between  md:py-2.5 md:px-20 pt-3 pb-1.5 px-2'>
        <Link to='/'><div className=' flex items-center gap-1'>
          ...
          <span className=' text-[#8000ff] font-bold md:text-[24px] text-[19px]'>Moviebee</span>
        </div>
        </Link>

        <div className=' text-white hidden md:flex items-center gap-3'>
          <Link to='/'><span className={` py-1.5 ${showcategory == 'Home' ? "border-b-[#8000ff] text-[#8000ff]" : " border-b-transparent text-white"} font-semibold border-b-[3.5px] cursor-pointer `} onClick={() => { setshowcategory('Home') }}> Home</span></Link>
          <Link to='/'><span className={` py-1.5 ${showcategory == 'Movies' ? "border-b-[#8000ff] text-[#8000ff]" : " border-b-transparent text-white"} font-semibold border-b-[3.5px] cursor-pointer `} onClick={() => { setshowcategory('Movies') }}>Movies</span></Link>
          <Link to='/'><span className={` py-1.5 ${showcategory == 'Shows' ? "border-b-[#8000ff] text-[#8000ff]" : " border-b-transparent text-white"} font-semibold border-b-[3.5px] cursor-pointer `} onClick={() => { setshowcategory('Shows') }}>TV Shows</span></Link>
          <Link to='/'><span className={` py-1.5 ${showcategory == 'Favorites' ? "border-b-[#8000ff] text-[#8000ff]" : " border-b-transparent text-white"} font-semibold border-b-[3.5px] cursor-pointer `} onClick={() => { setshowcategory('Favorites') }}>Favroites</span></Link>
        </div>
